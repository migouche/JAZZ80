...
use super::{
    AddressingMode, Flag, GPR, PrefixAddressing, RegisterPair, SystemRegister, Z80A, decode_opcode,
    flags,
};
        ...
        1 => match z {
            ...
            7 => match y {
                ...
                4 | 5 => {
                    ...
                    cpu.set_flag(false, Flag::H);
                    cpu.set_flag(false, Flag::N);
                    cpu.set_flag(x, Flag::X);
                    cpu.set_flag(y_flag, Flag::Y);
                }
...
pub(super) fn decode_unprefixed(cpu: &mut Z80A, opcode: u8, addressing: PrefixAddressing) {
    ...
    match x {
        0 => match z {
            ...
            2 => match (q, p) {
                ...
                (false, 2) => {
                    ...
                    let addr = cpu.fetch_word();
                    let transformed = cpu.transform_register(
                        AddressingMode::RegisterPair(RegisterPair::HL),
                        addressing,
                    );
            ...
            7 => match y {
                0 => {
                    ...
                    let a = cpu.get_register(GPR::A);
                    let (res, f) = rot::rlc(a);
                    cpu.set_register(GPR::A, res);
                    cpu.set_flag((f & flags::CARRY) != 0, Flag::C);
                    cpu.set_flag(false, Flag::N);
                    cpu.set_flag(false, Flag::H);
                }
                1 => {
                ...
                5 => {
                    test_log!(cpu, "CPL");
                    let result = cpu.get_register(GPR::A) ^ 0xFF;
                    cpu.set_register(GPR::A, result);
                    cpu.set_flag(true, Flag::N);
                    cpu.set_flag(true, Flag::H);
                    cpu.set_flag((result & flags::X) != 0, Flag::X);
                    cpu.set_flag((result & flags::Y) != 0, Flag::Y);
                }
                6 => {
                    test_log!(cpu, "SCF");
                    cpu.set_flag(true, Flag::C);
                    cpu.set_flag(false, Flag::N);
                    cpu.set_flag(false, Flag::H);
                    let a = cpu.get_register(GPR::A);
                    cpu.set_flag((a & flags::X) != 0, Flag::X);
                    cpu.set_flag((a & flags::Y) != 0, Flag::Y);
                }
                7 => {
                    test_log!(cpu, "CCF");
                    let current_carry = cpu.get_flag(Flag::C);
                    cpu.set_flag(!current_carry, Flag::C);
                    cpu.set_flag(false, Flag::N);
                    cpu.set_flag(current_carry, Flag::H);
                    let a = cpu.get_register(GPR::A);
                    cpu.set_flag((a & flags::X) != 0, Flag::X);
                    cpu.set_flag((a & flags::Y) != 0, Flag::Y);
                }
        ...
        3 => match z {
            0 => {
                test_log!(cpu, "RET cc[y]");
                let condition = cpu.table_cc(y);
                if cpu.evaluate_condition(condition) {
                    cpu.pc = cpu.pop();
                }
            }
            1 => match (q, p) {
                (false, _) => {
                    test_log!(cpu, "POP rp2[p]");
                    let rp = cpu.table_rp2(p);
                    let dest = cpu.transform_register(AddressingMode::RegisterPair(rp), addressing);
                    let value = cpu.pop();
                    cpu.ld_16(dest, AddressingMode::ImmediateExtended(value));
                }
                (true, 0) => {
                    test_log!(cpu, "RET");
                    cpu.pc = cpu.pop();
                ...
                (true, 2) => {
                    test_log!(cpu, "JP HL");
                    let src = cpu.transform_register(
                        AddressingMode::RegisterPair(RegisterPair::HL),
                        addressing,
                    );
                    cpu.pc = cpu.read_16(src);
                }
                (true, 3) => {
                    test_log!(cpu, "LD SP, HL");
                    let src = cpu.transform_register(
                        AddressingMode::RegisterPair(RegisterPair::HL),
                        addressing,
                    );
                    cpu.ld_16(AddressingMode::RegisterPair(RegisterPair::SP), src);
                }
                _ => unreachable!("Invalid q, p values"),
            },
            2 => {
                test_log!(cpu, "JP cc[y], nn");
                let condition = cpu.table_cc(y);
                let addr = cpu.fetch_word();
                if cpu.evaluate_condition(condition) {
                    cpu.pc = addr;
            ...
            3 => match y {
                ...
                2 => {
                    test_log!(cpu, "OUT (n), A");
                    let port = cpu.fetch() as u16;
                    let a = cpu.get_register(GPR::A);
                    cpu.write_io(port, a);
                }
                3 => {
                ...
                5 => {
                    test_log!(cpu, "EX DE, HL");
                    let de = cpu.get_register_pair(RegisterPair::DE);
                    let hl = cpu.get_register_pair(RegisterPair::HL);
                    cpu.set_register_pair(RegisterPair::DE, hl);
                    cpu.set_register_pair(RegisterPair::HL, de);
                }
                6 => {
                    test_log!(cpu, "DI");
                    cpu.iff1 = false;
                    cpu.iff2 = false;
                    cpu.iff_delay_count = 0;
                }
                7 => {
                    test_log!(cpu, "EI");
                    cpu.iff_delay_count = 2;
                }
                _ => unreachable!("Invalid y value"),
            },
            4 => {
                test_log!(cpu, "CALL cc[y], nn");
                let condition = cpu.table_cc(y);
                let addr = cpu.fetch_word();
                if cpu.evaluate_condition(condition) {
                    cpu.push(cpu.pc);
            ...
            5 => match (q, p) {
                (false, _) => {
                    test_log!(cpu, "PUSH rp2[p]");
                    let rp = cpu.table_rp2(p);
                    let src = cpu.transform_register(AddressingMode::RegisterPair(rp), addressing);
                    let value = cpu.read_16(src);
                    cpu.push(value);
                }
                (true, 0) => {
                    test_log!(cpu, "CALL nn");
                    let addr = cpu.fetch_word();
                    cpu.push(cpu.pc);
                    cpu.pc = addr;
                }
            ...
            6 => {
                test_log!(cpu, "ALU[y] n");
                let operation = cpu.table_alu(y);
                let n = cpu.fetch();
                cpu.alu_op(operation, n);
            }
            7 => {
                test_log!(cpu, "RST y*8");
                let rst_addr = y as u16 * 8;
                test_log!(cpu, "{:02X}h", rst_addr);
                cpu.push(cpu.pc);
                cpu.pc = rst_addr;
            }
            _ => unreachable!("Invalid z value"),
